#!/usr/bin/env python3
"""
Infrastructure as Code (IaC) Security Agent
Integrated with AI Orchestrator for detecting infrastructure misconfigurations
"""
...
class IaCAgent:
    """IaC security agent using AI orchestrator"""
    ...
    async def scan(self, repository_path: str, scan_config: Dict[str, Any]) -> Dict[str, Any]:
        """Perform IaC security scan using AI orchestrator"""
        scan_id = scan_config.get('scan_id', 'unknown')
        scan_type = scan_config.get('scan_type', 'iac')
        branch = scan_config.get('branch', 'main')
        
        logger.info(f"Starting IaC scan {scan_id} with AI orchestrator")
        
        try:
            # Configure scan to focus on infrastructure
            scan_config['focus_areas'] = ['infrastructure', 'terraform', 'cloudformation', 'kubernetes']
            ...
            # Extract IaC-specific findings
            iac_findings = self._extract_iac_findings(scan_result)
            ...
            return {
                ...
                'total_findings': len(iac_findings),
    ...
    def _extract_iac_findings(self, scan_result) -> List[Dict[str, Any]]:
        """Extract IaC-specific findings from scan result"""
